...
**資料庫**：`script-rehearsal-audio`，版本 **2**（從 v1 遷移）
...
// store: 'audioSegments'，keyPath: ['characterKey', 'globalIndex']
// index: 'byCharacter' on 'characterKey'
```

v2 的 `audioFiles` / `transcriptions` / `alignments` 三個 store **移除**（migration 跳過讀取，舊資料若存在則清空）。
...
| `app/page.tsx` | 設定頁底部插入 `<AudioManager />`（顯示錄音進度） |
